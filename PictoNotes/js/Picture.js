...
/** Droping the picture onto the "square"*/
...
function onDropTarget(obj, event,numPin) {
    ...
    if (event.dataTransfer) {
        ...
        if (!textData) {
            alert("An error occurs. Please close this widget and try again.");
        }
        textData = stringToXML(textData);
        var tmp = textData.getElementsByTagName("path")[0].firstChild.textContent;
		var extention = tmp.split(".")[(tmp.split(".").length)-1];
		var imp = document.getElementById("imp");
		//create a different tag depending on what is dropped on the widget
		switch (extention) {
			case "mp3":
				...
				if(inp){
					...
					//With this option, the song is played when the user click on the pin
					inp.value = inp.value + "<audio display = \"none\" Autoplay=\"autoplay\"><source src=\""+tmp+"\" type=\"audio/"+extention+"\" /> </audio>";
					//With this option, the software open a "play control"
					//inp.value = inp.value + "<audio controls=\"controls\"><source src=\""+tmp+"\" type=\"audio/"+extention+"\" /> </audio>";
					//With this option, the the song will loop
					//inp.value = inp.value + "<audio Loop=\"controls\"><source src=\""+tmp+"\" type=\"audio/"+extention+"\" /> </audio>";
				}
				break;
			...
			case "mpg":
				...
				if(inp){
					...
					var inp = document.getElementById("inp"+numPin);
					//With this option, the song is played when the user click on the pin
					inp.value = inp.value + "<video controls=\"controls\"><source src=\""+tmp+"\ /> </video>";
					//With this option, the software open a "play control"
					//inp.value = inp.value + "<audio controls=\"controls\"><source src=\""+tmp+"\" type=\"audio/"+extention+"\" /> </audio>";
					//With this option, the the song will loop
					//inp.value = inp.value + "<audio Loop=\"controls\"><source src=\""+tmp+"\" type=\"audio/"+extention+"\" /> </audio>";
			...
			case "gif":
			case "bmp":
				// if no image has been selected the dropped element become the new image
				if (imagePath == ""){
					imagePath = tmp;
