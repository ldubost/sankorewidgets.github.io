...
function growText(numPin){
	switch(pins[numPin][5]){
		case 16: pins[numPin][5] = 22; break;
		case 22: pins[numPin][5] = 36; break;
		case 36: pins[numPin][5] = 48; break;
		case 48: pins[numPin][5] = 16; break;
		default: pins[numPin][5] = 22; break;
	}
	//pins[numPin][5]++;
...
function createBubble(nbPin){
	...
	if (isEditor){
		...
		var colYellow = document.createElement('span');
		...
		tableColor.appendChild(colYellow);
		...
		yellow.setAttribute("onClick","changeColor("+nbPin+",'yellow')");
		...
		textSizeBigger.src = "images/aSize.png";			
		...
		tableTxt.style.backgroundColor = "#FFFFFF";
...
function addPin(event){	
	if (isEditor){
		...
		pins [nbPin]= [pos_x/image.width,pos_y/image.height,"","yellow","nonOriente",12];
		...
		if(pos_y <offsetPinY){
			pin.id = "pinr"+nbPin;
			pin.style.backgroundImage="url(images/pin-yellow-rev.png)";
			pin.style.width = 25
			pin.style.height = 25;
			...
			pin.id = "pin"+nbPin;
			pin.style.backgroundImage = "url(images/pin-yellow.png)";
			pin.style.width = 25;
			pin.style.height = 25;
