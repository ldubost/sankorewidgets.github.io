		...
		engine: function () {
			...
			return {
				...
				start: function () {
					...
					gamevars.yourbest = parseInt(XWUZ.cookie.get('bubble_bestscore'), 10) || 0;
				...
				gameover: function () {
					...
					if (gamevars.score > gamevars.yourbest) {
						XWUZ.say.alert('<h1>Partie terminée</h1>Le meilleur score est dépassé !');
						gamevars.yourbest = gamevars.score;
						XWUZ.cookie.set('bubble_bestscore', gamevars.yourbest);
					} else {
						XWUZ.say.alert('<h1>Game over</h1>Une nouvelle partie ?');
					}
				}
			};			
		}()
